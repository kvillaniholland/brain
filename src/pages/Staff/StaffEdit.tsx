...
const StaffEdit: React.FC<{ match: { params: { staffId: number } } }> = (props) => {
    const id = props.match.params.staffId;

    const staff = useProvider(() => Staff.getById(id))

    return (<>
        {staff ? <Page title={staff.first_name} toolbar={<IonButtons slot="start">
            <IonBackButton defaultHref={`/staff/${id}`}></IonBackButton>
        </IonButtons>}>
            <ModelForm provider={Staff} relationships={[...Staff.config.relationships]} model={staff} />
        </Page> : ''}
    </>
    );
};
