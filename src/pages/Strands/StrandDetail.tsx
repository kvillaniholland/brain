...
const StrandDetail: React.FC<{ match: { params: { strandId: number } } }> = (props) => {
    ...
    return (<>
        {strand ? <Page title={strand.name} toolbar={<IonButtons slot="start">
            <IonBackButton defaultHref="/strands"></IonBackButton>
        </IonButtons>}>
            <p>Room: {strand?.location.name}</p>
            <p>Period: {strand?.period.name}</p>
            <p>Staff: {strand?.staff.map(staff => staff.first_name).join(', ')}</p>
            <p>Students: {strand?.students.map(student => (`${student.first_name} ${student.last_name}`)).join(', ')}</p>
