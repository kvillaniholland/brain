...
const attachManyToMany = (
  thisSide: ModelConfig,
  thatSide: ModelConfig
) => async (thisSideId: number, thatSideId: number) => {
  const tableName = getManyToManyTable(thisSide, thatSide);
  await Database.from(tableName).upsert({
    [`${thatSide.type}_id`]: thatSideId,
    [`${thisSide.type}_id`]: thisSideId,
  });
};

const removeManyToMany = (
  thisSide: ModelConfig,
  thatSide: ModelConfig
) => async (thisSideId: number, thatSideId: number) => {
  const tableName = getManyToManyTable(thisSide, thatSide);
...
const getManyToManyIds = (
  thisSide: ModelConfig,
  thatSide: ModelConfig
) => async (sideTwoId: number): Promise<number[]> => {
  const tableName = getManyToManyTable(thisSide, thatSide);
  const { data } = await Database.from(tableName)
    .select(`${thatSide.type}_id`)
    .eq(`${thisSide.type}_id`, sideTwoId);
  return data!.map((item: any) => item[`${thatSide.type}_id`]);
};

const getManyToManyTable = (thisSide: ModelConfig, thatSide: ModelConfig) =>
  [thisSide.type, thatSide.type].sort().join("_");
...
const setManyToMany = (thisSide: ModelConfig, thatSide: ModelConfig) => async (
  parent: IModel,
  children: IModel[]
) => {
  ...
  await Promise.all(
    currentChildIds.map((id) =>
      !newChildIds.includes(id) ? remove(parent.id, id) : Promise.resolve()
    )
  );
  await Promise.all(
    newChildIds.map((id) =>
      !currentChildIds.includes(id) ? attach(parent.id, id) : Promise.resolve()
    )
  );
};
