  ...
  config.relationships.forEach((rel) => {
    provider[rel.name] =
      rel.type === Relationship.HasMany
        ? HasMany(config, { type: rel.model })
        : HasOne(config, { type: rel.model });
  });
  Providers[config.type] = provider;
  return provider as ModelProvider<T> & {
    [key: string]: RelationshipProvider<any>;
  };
};

const saveEntity = (model: ModelConfig, properties: string[]) => async (
  entity: IModel
) => {
  const tableName = getEntityTableName(model);
  const { data } = await Database.from(tableName)
    .upsert(only(entity, [...properties, "id"]))
    .single();
...
const getQuery = (config: ModelConfig, eager = false): string => {
  const propsQ = config.properties.reduce(
    (acc: string, curr: PropertyConfig) => {
      return `${acc},${curr.name}`;
    },
    "id"
  );

  return config.relationships.reduce(
    (acc: string, curr: RelationshipConfig) => {
      return `${acc},${curr.name}:${curr.model}s(${
        eager ? getQuery(Providers[curr.model].config) : "id"
      })`;
    },
    propsQ
  );
...
// UTILS
const only = (obj: any, keys: string[]) =>
  Object.keys(obj)
    .filter((key) => keys.includes(key))
    .reduce((acc: any, curr: string) => ({ ...acc, [curr]: obj[curr] }), {});
